...
/*                                                                            */
/*                                                        ::::::::            */
/*   get_new_position_s.c                               :+:    :+:            */
/*                                                     +:+                    */
/*   By: jaka <jaka@student.codam.nl>                 +#+                     */
/*                                                   +#+                      */
/*   Created: 2021/08/31 11:53:22 by jmurovec      #+#    #+#                 */
...
void	new_position_s_q1(float old_posX, float old_posY, t_window *w)
{
	...
	w->pos.my_x = w->pos.my_x - sin(w->pos.myrot * RAD) * STEP;
	w->pos.my_y = w->pos.my_y + cos(w->pos.myrot * RAD) * STEP;
...
void	new_position_s_q2(float old_posX, float old_posY, t_window *w)
{
	...
	fieldnr = (int)(floor(old_posX) + floor(old_posY) * w->map.nr_cols);
	w->pos.my_x = w->pos.my_x - sin((90 - (w->pos.myrot - 90)) * RAD) * STEP;
	w->pos.my_y = w->pos.my_y - cos((90 - (w->pos.myrot - 90)) * RAD) * STEP;
	if (w->pos.my_x - floor(w->pos.my_x) < NEAR_FLOOR)
		if (w->map.plan[fieldnr - 1] == '1')
			w->pos.my_x = floor(old_posX) - NEAR_FLOOR;
...
void	new_position_s_q3(float old_posX, float old_posY, t_window *w)
{
	...
	fieldnr = (int)(floor(old_posX) + floor(old_posY) * w->map.nr_cols);
	w->pos.my_x = w->pos.my_x + cos((90 - (w->pos.myrot - 180)) * RAD) * STEP;
	w->pos.my_y = w->pos.my_y - sin((90 - (w->pos.myrot - 180)) * RAD) * STEP;
...
void	new_position_s_q4(float old_posX, float old_posY, t_window *w)
{
	...
	fieldnr = (int)(floor(old_posX) + floor(old_posY) * w->map.nr_cols);
	w->pos.my_x = w->pos.my_x + sin((90 - (w->pos.myrot - 270)) * RAD) * STEP;
	w->pos.my_y = w->pos.my_y + cos((90 - (w->pos.myrot - 270)) * RAD) * STEP;
	if (w->pos.my_x - floor(w->pos.my_x) > NEAR_CEIL)
		if (w->map.plan[fieldnr + 1] == '1')
			w->pos.my_x = ceil(old_posX) - NEAR_FLOOR;
	if (w->pos.my_y - floor(w->pos.my_y) > NEAR_CEIL)
